...
void sampleInfo(string type, int latticeSize, unsigned long long numSpins, double interactionRange, unsigned long long nb, double initialSystemEnergy, unsigned long long nIters);
...
int main(int argc, char *argv[])
{
    ...
    int latticeSize = 9; //размер решетки latticeSize x latticeSize
    ...
    unsigned long long nIters = 100000; // число Монте-Карло шагов
    double T=0;
    ...
    if (rank == 0){
        ...
        sampleInfo(sys.type().toStdString(), latticeSize, sys.size(), sys.interactionRange(), nb, initialSystemEnergy, nIters);
    }
        ...
        for (int l=0; l < itersC; l++) {
            ...
            while(chaotic){
                ...
                for (int i = 0; i < nIntervals; i++) {
                    if (i == 0 && sys.E() < initialSystemEnergy) {
                        arrIntervals[i]++;
                        arrIntervalsEnergy[i] += sys.E();
                    }
                    else if ((sys.E() >= (initialSystemEnergy + (i * stepOfInterval))) && ((sys.E() < (initialSystemEnergy + ((i + 1) * stepOfInterval))))) {
                        arrIntervals[i]++;
                        arrIntervalsEnergy[i] += sys.E();
                    } else if ((i == (nIntervals - 1)) &&(sys.E() > (initialSystemEnergy + (i * stepOfInterval)))) {
                        arrIntervals[i]++;
                        arrIntervalsEnergy[i] += sys.E();
                    }
                }
                ...
                if (rank == 1)
                    if (iterations%100000 == 0){
                        cout << endl << iterations << " steps of " << nIters << endl;
                        cout << "Time: " << (double)clock() / 1000.0 << " sec." << endl;
                    }
            }
...
void sampleInfo(string type, int latticeSize, unsigned long long numSpins, double interactionRange, unsigned long long nb, double initialSystemEnergy, unsigned long long nIters) {
    ...
    info << "Monte Carlo Steps: " << nIters << endl;
